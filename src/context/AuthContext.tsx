...
type AuthContextProps = {
    ...
    updateProfile:(idUser:number,body:object ) => void
...
export const AuthProvider = ({ children }: any) => {
    ...
    const updateImg = async (email: string, file: object) => {
        ...
        if (data.status == 200) {
            dispatch({ type: 'updateUser', payload: { user: data.result, codeStatus: data.status } })
        } else {
    ...
    const updateProfile = async (idUser: number, body: object) => {

        //obtemos la url de la foto y se la pasamos aqui 

        const { data } = await apiDaily.put(`user/updateProfile/${idUser}`, body)
